        ...
        if (checkExecutedAsPlanned) {
            try {
                ...
                if(traceFailedBeforeAlertAction(trace)) {
                    ...
                    i.addAdditionalResultInfo("Workflow failed at Alert receiving action. Received: " + alertReceivingAction.getReceivedMessages().stream().map(ProtocolMessage::toString).collect(Collectors.joining(",")));
                }
                throw e;
            }
        }
