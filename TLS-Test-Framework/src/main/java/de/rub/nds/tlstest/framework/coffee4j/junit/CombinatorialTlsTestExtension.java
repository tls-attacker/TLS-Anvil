...
/**
 * The extension handling a {@link CombinatorialTest}. Creates one {@link
 * CombinatorialTestInvocationContext} for each test input.
 *
 * <p>This extension is more or less a copy of {@link
 * org.junit.jupiter.params.ParameterizedTestExtension} in the junit-jupiter-params project.
 */
public class CombinatorialTlsTestExtension extends CombinatorialTestExtension {
    ...
    private TlsTestCombinatorialTestNameFormatter createNameFormatter(Method testMethod) {
        final TestChooser combinatorialTest =
                findAnnotation(testMethod, TestChooser.class)
                        .orElseThrow(
                                () ->
                                        new JUnitException(
                                                "Illegal state: could not find combinatorial test annotation"));
        final String name =
                Preconditions.notBlank(
                        combinatorialTest.name().trim(),
                        () ->
                                "Configuration error: @"
                                        + TestChooser.class.getSimpleName()
                                        + " on method "
                                        + testMethod.getName()
                                        + " must be declared with a non-empty name.");
        return new TlsTestCombinatorialTestNameFormatter(name);
    }

    private TestTemplateInvocationContext createInvocationContext(
            TlsTestCombinatorialTestNameFormatter nameFormatter,
