...
/**
 * The manager that knows and manages all DerivationCategoryManager%s. It is used to collect the set
 * of DerivationParameter%s for a specified ModelType and ModelScope. Also it can used as a factory
 * to create DerivationParameter%s for given DerivationType%s.
 *
 * <p>The basic derivation type is registered by default.
 *
 * <p>The DerivationManager is a global Singleton.
 */
public class DerivationManager {
    ...
    public DerivationParameter getDerivationParameterInstance(DerivationType type) {
        ...
        LOGGER.error(
                "Derivations of type category {} were not registered in the DerivationManager.",
                type);
    ...
    public List<DerivationType> getDerivationsOfModel(
        ...
        for (Map.Entry<Class, DerivationCategoryManager> entry : categoryManagers.entrySet()) {
            derivationsOfModel.addAll(
                    entry.getValue().getDerivationsOfModel(derivationScope, baseModel));
        }
    ...
    public DerivationType getDerivationFromString(String derivationString) {
        ...
        if (splittedString.length != 2) {
            throw new IllegalArgumentException(
                    String.format(
                            "Illegal String format. Derivation format is '<TypeEnum>.<TypeName>' but '%s' was given.",
                            derivationString));
        }
        ...
        if (associatedClass == null) {
            ...
            throw new UnsupportedOperationException(
                    String.format(
                            "Derivations of type '%s' are not registered. Registered types are: %s",
                            derivationTypeEnum, String.join(", ", registeredTypes)));
        }
        ...
        try {
            ...
            throw new RuntimeException(
                    String.format("%s is no enum. This should not happen.", derivationTypeEnum));
        } catch (InvocationTargetException e) {
            if (e.getCause() instanceof IllegalArgumentException) {
                throw new IllegalArgumentException(
                        String.format(
                                "There is no value '%s' in enum %s.",
                                derivationTypeName, derivationTypeEnum));
            } else {
                throw new RuntimeException(
                        String.format("Unknown Exception was thrown.", derivationTypeEnum));
            }
    ...
    public synchronized void registerCategoryManager(
            Class derivationTypeCategory, DerivationCategoryManager categoryManager) {
        if (!DerivationType.class.isAssignableFrom(derivationTypeCategory)) {
            throw new IllegalArgumentException(
                    String.format(
                            "Passed derivationTypeCategory '%s' does not implement the DerivationType interface.",
                            derivationTypeCategory.toString()));
        }
        if (!derivationTypeCategory.isEnum()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Passed derivationTypeCategory '%s' is not an enum.",
                            derivationTypeCategory.toString()));
        }
        ...
        Collections.sort(
                ...
                (d1, d2) -> {
                    if ((d1 instanceof ConfigOptionDerivationType)
                            != (d2 instanceof ConfigOptionDerivationType)) {
                        if (d1 instanceof ConfigOptionDerivationType) {
                            return -1;
                        } else {
                            return 1;
