...
/**
 * The ParameterExtensionManager is a global singleton used to register, load and unload
 * ParameterExtensions.
 */
public class ParameterExtensionManager {
        ...
        if (availableExtensions.containsKey(parameterExtension.getIdentifier())) {
            if (parameterExtension != availableExtensions.get(parameterExtension.getIdentifier()))
                throw new IllegalArgumentException(
                        String.format(
                                "Tried to register Parameter Extension identifier '%s' twice with different Extension instances.",
                                parameterExtension.getIdentifier()));
        }
     ...
     * @param initData - Some data to initialize the extension. The expected data and type depends
     *     on the extension.
     */
    public synchronized void loadExtension(String identifier, Object initData) {
        if (!availableExtensions.containsKey(identifier)) {
            throw new IllegalArgumentException(
                    String.format(
                            "Parameter Extension identifier '%s' is not known by the ParameterExtensionManager. Have you registered it?",
                            identifier));
        }
