...
public class BasicDerivationManager implements DerivationCategoryManager {
    ...
    public DerivationParameter getDerivationParameterInstance(DerivationType type) {
        if (!(type instanceof BasicDerivationType)) {
            throw new IllegalArgumentException(
                    "This manager can only handle BasicDerivationTypes but type '"
                            + type
                            + "' was passed.");
        }
    ...
    private static List<DerivationType> getBasicDerivationsForServer(
        ...
        List<ExtensionType> supportedExtensions =
                TestContext.getInstance().getSiteReport().getSupportedExtensions();
        ...
        if (TestContext.getInstance()
                        .getSiteReport()
                        .getResult(AnalyzedProperty.HAS_GREASE_CIPHER_SUITE_INTOLERANCE)
                != TestResult.TRUE) {
        ...
        if (TestContext.getInstance()
                        .getSiteReport()
                        .getResult(AnalyzedProperty.HAS_GREASE_NAMED_GROUP_INTOLERANCE)
                != TestResult.TRUE) {
