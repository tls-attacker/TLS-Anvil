...
public class ParallelExecutorWithTimeout extends ParallelExecutor {
    ...
    /**
     * Ugly way to access the private addStateTask function of the parallel executor. TODO: Request
     * to make TLS-Attackers addStateTask method protected.
     */
    private Future<ITask> addStateTask(State state) {
        try {
            Method addStateTaskMethod =
                    ParallelExecutor.class.getDeclaredMethod("addStateTask", State.class);
            addStateTaskMethod.setAccessible(true);
            return (Future<ITask>) addStateTaskMethod.invoke(this, state);
        } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
            throw new UnsupportedOperationException(
                    "addStateTask of ParallelExecutor does not exist (anymore?).");
        }
    }

    /**
     * Ugly way to access the private addTask function of the parallel executor. TODO: Request to
     * make TLS-Attackers addTask method protected.
     */
    private Future<ITask> addTask(TlsTask task) {
        try {
            Method addTaskMethod =
                    ParallelExecutor.class.getDeclaredMethod("addTask", TlsTask.class);
            addTaskMethod.setAccessible(true);
            return (Future<ITask>) addTaskMethod.invoke(this, task);
        } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
            throw new UnsupportedOperationException(
                    "addTask of ParallelExecutor does not exist (anymore?).");
        }
    }
