...
public class EncryptedExtensions extends TlsGenericTest {
    ...
    @AnvilTest(description = "Send an Encrypted Extensions Message with a modified length value (-1)")
    ...
    @AnvilTest(
            description =
                    "Send an Encrypted Extensions Message with a modified Extensions list length value (+1)")
    @ModelFromScope(modelType = "LENGTHFIELD")
    @MessageStructureCategory(SeverityLevel.MEDIUM)
    @HandshakeCategory(SeverityLevel.MEDIUM)
    @AlertCategory(SeverityLevel.LOW)
