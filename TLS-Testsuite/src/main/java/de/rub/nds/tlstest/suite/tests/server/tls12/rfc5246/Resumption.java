...
public class Resumption extends Tls12Test {
    ...
    @TlsTest(description = "A server that implements this extension MUST NOT accept the request " +
            ...
            "different name.", securitySeverity = SeverityLevel.MEDIUM)
    ...
    public void rejectSniDisparityResumption(WorkflowRunner runner) {
        ...
        runner.setStateModifier(s -> {
            ...
            sni2.setServerNameList(new ArrayList<ServerNamePair>(){{ add(pair);}});
    ...
    @TlsTest(description = "When resuming a session, the server MUST " +
            "NOT include a server_name extension in the server hello.", interoperabilitySeverity = SeverityLevel.LOW)
    @RFC(number = 6066, section = "3.  Server Name Indication")
    @MethodCondition(method = "supportsResumptionAndSniActive")
    ...
    public void rejectResumptionAfterFatalPostHandshake(WorkflowRunner runner) {
        Config c = this.getConfig();
        runner.generateWorkflowTraceUntilLastMessage(WorkflowTraceType.FULL_RESUMPTION, HandshakeMessageType.SERVER_HELLO);
