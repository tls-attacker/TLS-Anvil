...
public class EarlyData extends Tls13Test {
    ...
    public void selectedFirstIdentity(WorkflowRunner runner) {
    ...
    public void cipherSuiteDisparity(WorkflowRunner runner) {
        Config c = this.getConfig();
        c.setAddPSKKeyExchangeModesExtension(true);
        c.setAddPreSharedKeyExtension(true);
        c.setAddEarlyDataExtension(true);
    ...
    @RFC(number = 8446, section = "4.2.10 Early Data Indication")
    @MethodCondition(method = "supports0rtt")
    public void tlsVersionDisparity(WorkflowRunner runner) {
        Config c = this.getConfig();
        c.setAddPSKKeyExchangeModesExtension(true);
        c.setAddPreSharedKeyExtension(true);
        c.setAddEarlyDataExtension(true);
    ...
    public void invalidCiphertext(WorkflowRunner runner) {
        ...
        runner.setStateModifier(s -> {
                ...
                cHello.getSendMessages().remove(s.getWorkflowTrace().getFirstSendMessage(ApplicationMessage.class));
                
                Record helloRecord = new Record();
                Record earlyRecord = new Record();
                earlyRecord.setComputations(new RecordCryptoComputations());
                earlyRecord.getComputations().setCiphertext(Modifiable.xor(new byte[]{0x01}, 0));
                ...
                s.getWorkflowTrace().getLastSendingAction().getSendRecords().add(helloRecord);
                s.getWorkflowTrace().getLastSendingAction().getSendRecords().add(earlyRecord);
        return null;
        });
    ...
    public void invalidAuthTag(WorkflowRunner runner) {
        ...
        runner.setStateModifier(s -> {
                ...
                Record earlyRecord = new Record();
                earlyRecord.setComputations(new RecordCryptoComputations());
                earlyRecord.getComputations().setAuthenticationTag(Modifiable.xor(new byte[]{0x01}, 0));
